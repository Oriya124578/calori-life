...
const folderPath = 'C:\\Users\\turhv\\OneDrive\\שולחן העבודה\\Studies\\year 1\\semester 2\\לוגיקה ותורת הקבוצות\\מבחנים';
...
async function run() {
  ...
  for (const fileName of files) {
    ...
    // Add to Firestore
    const taskId = `${courseId}_gpast_exams_${Date.now()}_${idx}`;
    const docData = {
      id: taskId,
      courseId,
      scope: 'global',
  ...
  console.log(`\nSuccessfully uploaded ${idx} exams/quizzes to Logic.`);
}
